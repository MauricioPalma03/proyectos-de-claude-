...
# rompería la propia declaración `window.__SEED_DATA_B64GZ__ = ...`.
bootstrap = (
    '<script>\n' + xlsx_lib + '\n</script>\n'
    '<script>\n'
...
# placeholders __SEED_DATA_VALUE__/__SELF_TEMPLATE_VALUE__ todavía SIN reemplazar
# — es el molde que el propio archivo usa para generar una copia nueva de sí mismo
# cuando alguien actualiza los datos (no cambia entre actualizaciones, siempre es
# el mismo "código" de la app).
self_template_b64gz = gzip_b64_str(html)

seed_data = json.load(open(SEED_DATA))
seed_data_b64gz = gzip_b64_str(json.dumps(seed_data, ensure_ascii=False))
